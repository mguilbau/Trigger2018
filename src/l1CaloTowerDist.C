...
int l1CaloTowerDist(const std::string inFileName)
{
  ...
  for(Int_t entry = 0; entry < nEntries; ++entry){
    ...
    for(unsigned int i = 0; i < towers_->iet.size(); ++i){
      ...
      if(towers_->iem.at(i) >= l1PtLow && towers_->iem.at(i) < l1PtHi) l1CaloTowerPt_EM_p->Fill(towers_->iem.at(i), 1./nEntries);
      if(towers_->ihad.at(i) >= l1PtLow && towers_->ihad.at(i) < l1PtHi) l1CaloTowerPt_HAD_p->Fill(towers_->ihad.at(i), 1./nEntries);
