...
int l1OfflineSubtract(std::vector<std::string> inFileName, std::vector<std::string> forestFileName, const bool doWeights, const std::string extraTag, std::vector<std::string> subTypeStr)
{
  ...
  l1ThreshPtBins[0] = l1ThreshPt[0]/2. - (l1ThreshPt[1]/2. - l1ThreshPt[0]/2.)/2.;
  ...
  for(Int_t i = 0; i < nL1Thresh; ++i){
    ...
    std::string l1Str = "p_{T,L1} > " + prettyString(l1ThreshPt[i]/2, 1, false);
  ...
  for(Int_t cI = 0; cI < nRhoBins+1; ++cI){
	...
	triggerEta_L1Pt_h[cI][sI][lI] = new TH1F(("triggerEta_" + centStr + "_" + subType[sI] + "_L1Pt" + prettyString(l1ThreshPt[lI]/2., 1, true) + "_h").c_str(), (centStr + ";L1 Jet #eta (L1 p_{T} > " + prettyString(l1ThreshPt[lI]/2., 1, false) + ");Counts").c_str(), 85, -42.5, 42.5);
    ...
    for(Int_t jI = 0; jI < nJetTrees; ++jI){
      for(Int_t lI = 0; lI < nL1Thresh; ++lI){       
	jetEta_L1Pt_h[cI][jI][lI] = new TH1F(("triggerEta_" + centStr + "_" + jetNames.at(jI) + "_JetPt" + prettyString(l1ThreshPt[lI]/2., 1, true) + "_h").c_str(), (centStr + ";Jet #eta (p_{T} > " + prettyString(l1ThreshPt[lI]/2., 1, false) + ");Counts").c_str(), 101, -5.1, 5.1);
  ...
  for(Int_t rI = 0; rI < nRhoBins+1; ++rI){
	...
	std::cout << "  " << l1ThreshPt[lI]/2 << ": " << triggerFires[rI][sI][lI] << "/" << triggerOpp[rI][sI][lI] << " ( " << prettyString(((Double_t)triggerFires[rI][sI][lI])/((Double_t)triggerOpp[rI][sI][lI]), 3, false) << ")" << std::endl;
      }
